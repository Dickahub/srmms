-- Extend the audit trail to cover repair_logs entries.
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'repair_logs' THEN
    v_label := COALESCE((to_jsonb(COALESCE(NEW, OLD))->>'action'), v_entity_id::text);
  ELSE
  ...
  VALUES (v_actor, v_email, v_action,
          CASE TG_TABLE_NAME
            ...
            WHEN 'repair_logs'  THEN 'repair_log'
            ELSE TG_TABLE_NAME
          END,
